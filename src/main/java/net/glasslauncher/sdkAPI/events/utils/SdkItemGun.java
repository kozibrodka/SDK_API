...
public abstract class SdkItemGun extends SdkItemCustomUseDelay {
                              ...
                              float f3, float f4)
    {
        if(!mod_SdkGuns.reloadTimes.containsKey(entity))
        {
            ...
            {
                if(!(entity instanceof EntityPlane || entity instanceof EntityVehicle || entity instanceof EntityPlaneNew)) { //addon
            ...
            if(i > 0)
            {
                ...
                if(entity instanceof PlayerBase)
                {
                    ...
                    if(!entity.method_1373()) //isSneaking
                    {
                        d *= 2D;
                        d1 *= 2D;
                        if(this instanceof SdkItemGunMinigun)
                        {
                            d *= 2D;
                            d1 *= 2D;
                        }
                    ...
                    entity.pitch -= d;
                    entity.yaw += d1;
                    if(i == 2 && !(this instanceof SdkItemGunMinigun) && !(this instanceof SdkItemGunLaser))
                    {
